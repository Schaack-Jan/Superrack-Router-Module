...
// Finds the rack whose mapped value matches the incoming mixer channel.
// rackMap is either an array indexed by rack ID or an object keyed by rack ID,
// with entries of shape { id, value }.
function resolveRackForChannel(rackMap, channel) {
	if (Array.isArray(rackMap)) {
		for (let i = 1; i < rackMap.length; i++) {
			const entry = rackMap[i]
			if (entry && entry.value === channel) {
				return entry.id ?? i
			}
		}
	} else if (rackMap && typeof rackMap === 'object') {
		for (const [key, entry] of Object.entries(rackMap)) {
			if (entry && entry.value === channel) {
				return entry.id ?? parseInt(key, 10)
			}
		}
	}
	return null
...
module.exports = { validateRackMidiMap, parseMidiMapString, applyMidiStepToVariables, resolveRackForChannel }
